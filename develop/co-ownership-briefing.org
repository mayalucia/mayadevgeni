#+title: Co-Ownership Briefings: Teaching Your Machine Collaborator About Your Project
#+subtitle: A Report on Designing Artifacts for Human–Machine Co-Ownership
#+author: mu2tau & claude
#+date: 2025-07-28
#+property: header-args :tangle no
#+property: header-args:markdown :mkdirp yes

* Preface: What This Document Is

  This report distills a design conversation between a physicist and an LLM
  collaborator. The question was deceptively simple: /what does it mean for a
  machine to co-own a codebase with a human?/
  ...
  /co-ownership briefings/ — project-level documents that orient a machine
...
* The Problem: Ownership Without Persistence

  When a human /owns/ a codebase, ownership lives in their head. They carry a
  ...
  line because they hold the /map/. The code on disk is the /territory/; the
  owner holds the map.
  ...
  /reconstruct/ a working model from the artifacts. The difference is that
  humans reconstruct slowly (days, weeks) while machines reconstruct quickly
  (seconds) — /if/ the artifacts support reconstruction.

  This reframes the question entirely:

  #+begin_quote
  Co-ownership is not a property of the machine.
  It is a property of the /artifact/.
  #+end_quote

  An artifact is /co-ownable/ when it carries enough structure and context that
...
* The Dual-Channel Principle
  ...
  + /Prose/ :: The human reads the narrative and reconstructs the argument:
  ...
  + /Code/ :: The machine reads the source blocks and reconstructs the
    architecture: what types exist, how they compose, what invariants hold.
    Good code lets a machine reproduce the manuscript's /computational/
    argument.

  Neither channel is subordinate. The prose is not "comments for the code."
  The code is not "illustration for the prose." They are /parallel
  representations of the same argument/, optimized for different readers.
...
* Three Formal Channels
  ...
  |-----------+----------+-----------------------------------+----------------------------------------------------|
  | /Spec/    | Haskell  | Machine + mathematically-inclined | Algebraic structure, type propositions, composition |
  | /Impl/    | C++/Rust | Machine + GPU                     | Executable realization, memory layout, performance  |
  | /Explore/ | Python   | Human scientist at the REPL       | Interactive experimentation, parameter sweeps       |

  The crucial move is recognizing Haskell (or a similar expressive language)
  as a /reasoning language/, not an implementation language. It doesn't need
  to render pixels or talk to the GPU. It needs to express, precisely and
  verifiably, the /algebraic structure/ of the system.

** Haskell as Specification

   Why Haskell for specification rather than pseudocode or prose?

   + /It compiles./ Unlike prose, which can be vague or self-contradictory,
     Haskell's type checker enforces coherence. If you say stages compose,
     the types must actually compose.
   + /It is concise./ The entire pipeline architecture — types, relationships,
     composition laws — might fit in a few hundred lines. That is a compressed,
     navigable map.
   + /It is verifiable./ You can write QuickCheck properties for algebraic
     laws (associativity of composition, functor laws). The specification
     tests itself.

   A Haskell spec file serves as the machine collaborator's /first read/
   ...
   Even with =undefined= implementations, this /type-checks/ — the
...
* Types as Propositions: The Reasoning Layer
  ...
  Why should a scientist care? Because /type signatures encode scientific claims/.
...
** What Types Rule Out
   ...
   What a type /prevents/ is as important as what it /allows/. If you write:
   ...
   You /cannot/ pass a =Stage<Mesh, Colors>= where =Stage<Mesh, Normals>= is
...
** Types as Scientific Claims
   ...
   signatures is reading the /argument structure/ of the pipeline — compressed,
...
** The Practical Discipline
   ...
   2. Discuss what it /means/ (what proposition it encodes)
   3. Note what it /rules out/ (what invalid compositions become unrepresentable)

   The prose explains to the human /why the proposition matters/. The type
   explains to the machine /what the proposition is/. Same argument, dual
   channels.
...
** 2. Intention Near Implementation
   ...
   + In pure source: comments explain /why/, not /what/
   + Design decisions belong near the code they affect, not in distant docs

** 3. Compositional Structure with Clear Interfaces
...
** 4. Tests as Executable Specifications
   ...
   + Tests are not just quality assurance — they are the machine's /calibration
     protocol/
   + Write tests that express /what should be true/, not just /what currently
     passes/
   + Property-based tests (QuickCheck, Hypothesis) encode algebraic laws

** 5. Bootstrap Documents

   A /bootstrap document/ is what the machine reads first when entering a
   ...
   + The tangleable templates in §6 of this report produce these

** 6. Explicit Type Signatures
   ...
   | /Order/   | Document flow        | Build dependency graph               |
   | /Valid?/  | Argument coherence   | Compilation + test passage           |
   | /Laws/    | Stated in prose      | Encoded in types + property tests    |

* Writing the Briefing: Composable Templates

   The templates below are composable. You can tangle them directly for a
   quick start, or use the individual sections (§6.3) as building blocks for
   a custom briefing.
   ...
   /Tangling/: Each template has a =:tangle= header. Run =org-babel-tangle=
   to produce the =.md= files. Adjust the tangle paths to match your project
   structure.

** Template: Project-Level Briefing

#+begin_src markdown :tangle templates/project-briefing-CLAUDE.md
...
** Template: Module-Level Briefing

#+begin_src markdown :tangle templates/module-briefing-CLAUDE.md
...
#+begin_src markdown :tangle templates/sections/co-ownership.md
...
*** Types-as-Propositions Section

#+begin_src markdown :tangle templates/sections/types-as-propositions.md
...
*** Three-Channel Section

#+begin_src markdown :tangle templates/sections/three-channels.md
### Three Formal Channels
...
* Closing: The Artifact Teaches Both
  ...
  orients a fresh session. Together, they make the project a /shared ground/
  where human intuition and machine traversal reinforce each other.

  Co-ownership also implies /co-responsibility/. The machine collaborator that
  ...
  The templates in §6 give you a starting point. Adapt them to your project.
  The principles in §1–5 tell you /why/ each section exists. The types-as-
  propositions framing (§4) tells you how to make your code carry its own
  argument.
...
* Appendix: A Worked Example                                      :noexport:

  The following are the actual =CLAUDE.md= files produced for the =MayaLucIA=
  project, a scientific computing framework for understanding natural phenomena
  through iterative modeling and visualization.

** Project-Level: =MayaLucIA/CLAUDE.md=

   This briefing sits at the framework root. Any agent working anywhere under
   =MayaLucIA/= inherits it. It establishes identity, philosophy, the
   co-ownership principle, and core conventions.

   Key design choices:
   + Leads with /Foundational Principles/ (co-ownership, guiding philosophy)
     before conventions — values before rules
   + Each principle is compressed to a paragraph with a pointer to the full
     treatment — the briefing is a map, not a textbook
   + Conventions section covers the non-negotiables: ORG as medium, literate
     programming, naming rules
   + Architecture section gives just enough structure to orient (directory
     tree or table)
   + Ends with a reading list ordered by importance

** Module-Level: =MayaPortal/CLAUDE.md=

   This briefing sits inside the visualization module. An agent here picks up
   /both/ this and the project-level briefing (by directory walk). It adds
   module-specific context without repeating project-level principles.

   Key design choices:
   + Opens with the three-channel table (Haskell/C++/Python) — this is the
     module's distinctive architectural feature
   + References the project-level co-ownership doc rather than restating it
   + Includes module-specific conventions (lesson structure, build system,
     the three-face protocol)
   + Keeps to ~100 lines — a briefing, not documentation
